...
export default function LeaveTypeForm({ closeForm, initialData }) {
  const [paid, setPaid]                 = useState(initialData?.paid ?? true);
  const [carryForward, setCarryForward] = useState(initialData?.carryForward ?? false);
  const [affectsSalary, setAffects]     = useState(initialData?.affectsSalary ?? false);
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl dark:bg-gray-800">
        ...
        <form className="flex flex-col gap-4 px-6 py-6">
          <div className="flex flex-col gap-1.5">
            ...
            <CustomInput id="leaveName" defaultValue={initialData?.name} placeholder="e.g. Annual Leave" required />
          </div>
          <div className="flex flex-col gap-1.5">
            <CustomLabel htmlFor="annualLimit">Annual Limit (days)</CustomLabel>
            <CustomInput id="annualLimit" type="number" min={0}
              defaultValue={initialData?.annualLimit}
              placeholder="0 = unlimited" required />
          </div>
          <div className="flex flex-col gap-3 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-600 dark:bg-gray-700">
            {[
              ...
              { label: "Carry Forward",      val: carryForward, set: setCarryForward },
              { label: "Affects Salary",     val: affectsSalary, set: setAffects },
            ].map(({ label, val, set }) => (
