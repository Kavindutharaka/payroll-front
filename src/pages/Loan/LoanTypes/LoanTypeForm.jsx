...
export default function LoanTypeForm({ closeForm, initialData }) {
  return (
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl dark:bg-gray-800">
        <div className="flex items-center justify-between rounded-t-2xl bg-purple-600 px-6 py-4">
          <h3 className="text-lg font-semibold text-white">
            {initialData ? "Edit Loan Type" : "Create Loan Type"}
          </h3>
        ...
        <form className="flex flex-col gap-4 px-6 py-6">
          <div className="flex flex-col gap-1.5">
            ...
            <CustomInput id="loanName" defaultValue={initialData?.name} placeholder="e.g. Staff Loan" required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <CustomLabel htmlFor="interestRate">Interest Rate (%)</CustomLabel>
              <CustomInput id="interestRate" type="number" step="0.5" min="0"
                defaultValue={initialData?.interestRate} placeholder="0 = interest free" required />
            </div>
            <div className="flex flex-col gap-1.5">
              <CustomLabel htmlFor="maxAmount">Max Amount (Rs.)</CustomLabel>
              <CustomInput id="maxAmount" type="number"
                defaultValue={initialData?.maxAmount} placeholder="e.g. 500000" required />
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <CustomLabel htmlFor="installmentMethod">Installment Method</CustomLabel>
            <CustomSelect id="installmentMethod" defaultValue={initialData?.installmentMethod ?? "Equal Monthly"}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <CustomSelect id="linkedComponent" defaultValue={initialData?.linkedComponent ?? ""}>
